"""
Kullanıcı Belleği — Atlas
==========================
Oturumlar arası kalıcı JSON bellek.
Kategoriler: identity | preferences | projects | notes | whatsapp_contacts
...
MEMORY_FILE = Path("workspace/user_memory.json")
...
def save_entry(category: str, key: str, value: str) -> str:
    """Tek kayıt ekle/güncelle. Supervisor tool-call'larından kullanılır."""
    if not key or not value:
        return "key ve value boş olamaz."
...
def _entry_matches(needle: str, category: str, item_key: str, item_value) -> bool:
    ...
    if any(needle in hay for hay in haystacks):
        return True
    tokens = [t for t in _tokenize(needle) if len(t) >= 3]
    if not tokens:
        return False
    entry_tokens: list[str] = []
    for hay in haystacks:
        entry_tokens.extend(_tokenize(hay))
    matched = sum(
        1 for t in tokens
        if any(t in et or et in t for et in entry_tokens)
    )
    return matched >= min(2, len(tokens)) if len(tokens) > 1 else matched == 1


def delete_memory(category: str = "", key: str = "", match_text: str = "") -> str:
    mem = load_memory()
    if not mem:
        return "Hafızada silinecek kayıt yok."
    ...
    return "Eşleşen hafıza kaydı bulamadım."


def format_memory_for_prompt(memory: dict | None = None) -> str:
    """Hafızayı supervisor/agent system prompt'una eklemek için formatla."""
    if memory is None:
        memory = load_memory()
    if not memory:
        return ""
    lines = ["[KULLANICI HAKKINDA KAYITLI BİLGİLER]"]
        ...
        if isinstance(items, dict):
            for key, val in items.items():
                if category == "whatsapp_contacts" and isinstance(val, dict):
                    ...
                    alias_str = f" (takma adlar: {', '.join(str(a) for a in aliases)})" if aliases else ""
...
def get_memory_summary() -> dict:
    """UI için hafıza özeti döndür."""
    mem = load_memory()
    result = {"total_entries": 0, "categories": {}}
    for cat, items in mem.items():
        if isinstance(items, dict):
            result["categories"][cat] = list(items.keys())
            result["total_entries"] += len(items)
        else:
            result["categories"][cat] = [str(items)]
